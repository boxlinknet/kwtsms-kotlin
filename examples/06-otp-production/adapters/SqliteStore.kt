 ...
 *
 * Uses WAL mode for concurrent reads. Prepared statements for all queries.
 *
...
class SqliteStore(dbPath: String) : OtpStore {
    ...
    override fun findLatest(phone: String): OtpRecord? {
        val stmt = conn.prepareStatement("SELECT * FROM otp_records WHERE phone = ?")
    ...
    override fun incrementAttempts(phone: String) {
        val stmt = conn.prepareStatement("UPDATE otp_records SET attempts = attempts + 1 WHERE phone = ?")
        stmt.setString(1, phone)
        stmt.executeUpdate()
    }

    override fun markUsed(phone: String) {
        val stmt = conn.prepareStatement("UPDATE otp_records SET used = 1 WHERE phone = ?")
        stmt.setString(1, phone)
        stmt.executeUpdate()
    }

    override fun delete(phone: String) {
        val stmt = conn.prepareStatement("DELETE FROM otp_records WHERE phone = ?")
        stmt.setString(1, phone)
        stmt.executeUpdate()
    }

    override fun countByPhone(phone: String, sinceMs: Long): Int {
        val stmt = conn.prepareStatement("SELECT COUNT(*) FROM otp_records WHERE phone = ? AND created_at >= ?")
        stmt.setString(1, phone)
    ...
    override fun countByIp(ip: String, sinceMs: Long): Int {
        val stmt = conn.prepareStatement("SELECT COUNT(*) FROM ip_usage WHERE ip = ? AND created_at >= ?")
        stmt.setString(1, ip)
        stmt.setLong(2, sinceMs)
        val rs = stmt.executeQuery()
        return if (rs.next()) rs.getInt(1) else 0
    ...
    override fun recordIpUsage(ip: String) {
        val stmt = conn.prepareStatement("INSERT INTO ip_usage (ip, created_at) VALUES (?, ?)")
        stmt.setString(1, ip)
        stmt.setLong(2, System.currentTimeMillis())
        stmt.executeUpdate()
        // Cleanup old entries
        val cleanup = conn.prepareStatement("DELETE FROM ip_usage WHERE created_at < ?")
        cleanup.setLong(1, System.currentTimeMillis() - 7200_000)
